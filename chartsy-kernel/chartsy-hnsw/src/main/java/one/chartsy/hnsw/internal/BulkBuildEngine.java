...
public final class BulkBuildEngine {
    ...
    private HnswIndex buildFromBatches(List<long[]> idsList, List<double[][]> vectorsList) {
        ...
        DefaultHnswIndex index = DefaultHnswIndex.newForBulk(config);
        if (total == 0) {
    ...
    private static final class BulkBuilder {
        ...
        private void buildLevel(int level, int[] nodes) {
            ...
            if (seedSize > 0) {
                ...
                int[] seedBlock = Arrays.copyOfRange(nodes, 0, seedSize);
            ...
            int currentSize = seedSize;
            while (currentSize < nodes.length) {
                int blockSize = nextBlockSize(level, currentSize);
                int end = Math.min(nodes.length, currentSize + blockSize);
                processBlock(level, Arrays.copyOfRange(nodes, currentSize, end), false);
                currentSize = end;
            }
        }

        private int nextBlockSize(int level, int currentGraphSize) {
            if (level != 0)
                return options.blockSizeUpper;

            // Dynamically grow the block size as the graph grows.
            // This prevents large blocks from overwhelming the small seeded graph.
            int dynamic = Math.max(1024, currentGraphSize / 2);
            return Math.min(options.blockSizeL0, dynamic);
        }
        ...
        private void phaseB(BlockResult result) {
            ...
            for (int i = 0; i < result.nodes.length; i++) {
                ...
                for (int j = 0; j < count; j++) {
                    ...
                    incoming.computeIfAbsent(neighbor, _ -> new IntArrayList()).add(nodeId);
                }
            }
