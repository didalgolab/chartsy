...
final class SearchScratch {
    ...
    SearchScratch(int initialNodes, int initialEf) {
    ...
    void ensure(int nodeCapacity, int bufferCapacity) {
        ensureVisitMark(nodeCapacity);
        ensureBuffers(bufferCapacity);
    }

    void reset(int nodeCapacity, int efSearch) {
    ...
    DoubleIntMinHeap candidates() {
        return candidates;
    }

    BoundedMaxHeap results() {
        return results;
    }

    int[] tmpNodes(int required) {
        ensureBuffers(required);
        return tmpNodes;
    }

    double[] tmpDistances(int required) {
        ensureBuffers(required);
        return tmpDistances;
    }

    int[] tmpNodesSecondary(int required) {
        ensureBuffers(required);
