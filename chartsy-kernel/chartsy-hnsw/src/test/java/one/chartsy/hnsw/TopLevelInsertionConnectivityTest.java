...
class TopLevelInsertionConnectivityTest {
    ...
    void nodePromotedAboveCurrentMaxLevelObtainsBaseNeighbors() throws Exception {
        ...
        Field idToInternalField = DefaultHnswIndex.class.getDeclaredField("idToInternal");
        idToInternalField.setAccessible(true);
        Long2IntOpenHashMap idToInternal = (Long2IntOpenHashMap) idToInternalField.get(internal);
        ...
        for (int iteration = 0; iteration < 500 && !verifiedPromotion; iteration++) {
            ...
            if (previousMaxLevel >= 0 && maxLevel > previousMaxLevel) {
                int internalId = idToInternal.get(id);
