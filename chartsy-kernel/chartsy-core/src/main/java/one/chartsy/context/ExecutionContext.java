...
/**
 * Object representing a context for specific operations. It is a thin wrapper
 * for a map that allows optionally for type-safe reads and dirty checking by
 * setting a 'dirty' flag whenever any put is called.
 * <p>
 * Note that putting {@code null} value is equivalent to removing the entry for the
 * given key.
 *
 * @author Lucas Ward
...
public class ExecutionContext implements Serializable {
	...
	private final ThreadLocal<Map<String, LocalDate>> parsingLocalDateCache = ThreadLocal.withInitial(HashMap::new);

	private final ThreadLocal<Map<String, LocalTime>> parsingLocalTimeCache = ThreadLocal.withInitial(HashMap::new);
	 ...
	 *
	 * @param context containing the entries to be copied to this current context.
	 */
	...
	/**
	 * Parses a {@link LocalDate} from the given character sequence using the given formatter,
     * utilizing a shared cache to improve performance for repeated parsing operations.
	 *
	 * <p>If the parsed result is not in the cache, it is parsed using the formatter and
     * then cached for future use within this ExecutionContext.</p>
	 *
	 * @param str the character sequence to parse
	 * @param dtf the formatter to use for parsing
	 * @return the parsed {@link LocalDate}
	 * @throws DateTimeParseException if the character sequence cannot be parsed
	 */
	public LocalDate getCachedLocalDate(CharSequence str, DateTimeFormatter dtf) throws DateTimeParseException {
		var cache = parsingLocalDateCache.get();

		@SuppressWarnings("SuspiciousMethodCalls")
		var cached = cache.get(str);
		if (cached == null) {
			String key = str.toString();
			cache.put(key, cached = LocalDate.parse(key, dtf));
		}
		return cached;
	}

	/**
	 * Parses a {@link LocalTime} from the given character sequence using the given formatter,
     * utilizing a shared cache to improve performance for repeated parsing operations.
	 *
	 * <p>If the parsed result is not in the cache, it is parsed using the formatter and
     * then cached for future use within this ExecutionContext.</p>
	 ...
	 * @throws DateTimeParseException if the character sequence cannot be parsed
	 */
	public LocalTime getCachedLocalTime(CharSequence str, DateTimeFormatter dtf) throws DateTimeParseException {
		var cache = parsingLocalTimeCache.get();

		@SuppressWarnings("SuspiciousMethodCalls")
		var cached = cache.get(str);
		if (cached == null) {
			String key = str.toString();
			cache.put(key, cached = LocalTime.parse(key, dtf));
		}
		return cached;
	}
