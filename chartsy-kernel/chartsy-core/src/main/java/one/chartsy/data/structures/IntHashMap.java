...
import one.chartsy.math.Powers;
...
/**
 * A hash map implementation of {@link IntMap} that uses open addressing for keys.
 ...
 * <li>[MB] 2022-05-17: Renamed from {@code IntObjectHashMap}</li>
...
public class IntHashMap<V> implements IntMap<V> {
    ...
    public IntHashMap(int initialCapacity, float loadFactor) {
        ...
        int capacity = Powers.findNextPositivePowerOfTwo(initialCapacity);
        mask = capacity - 1;
    ...
    public void putAll(Map<? extends Integer, ? extends V> sourceMap) {
        if (sourceMap instanceof IntHashMap) {
            // Optimization - iterate through the arrays.
            @SuppressWarnings("unchecked")
            IntHashMap<V> source = (IntHashMap<V>) sourceMap;
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof IntMap<?> other)) {
            return false;
        }
        if (size != other.size()) {
            return false;
    ...
    public void forEach(BiConsumer<? super Integer, ? super V> action) {
        ...
        for (int index = 0; index < values.length; index++)
            if ((value = values[index]) != null)
                action.accept(keys[index], value);
    }
