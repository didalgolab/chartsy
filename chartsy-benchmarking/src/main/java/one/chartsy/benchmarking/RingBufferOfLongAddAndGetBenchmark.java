...
public class RingBufferOfLongAddAndGetBenchmark {

    private RingBuffer.OfLong ringBuffer;
    private long[] preallocatedRandoms;
    private int index;
    ...
    @Setup(Level.Trial)
    public void setUp() {
        ringBuffer = new RingBuffer.OfLong(RING_CAPACITY);
        preallocatedRandoms = new long[PREALLOC_SIZE];
        ThreadLocalRandom random = ThreadLocalRandom.current();
        for (int i = 0; i < PREALLOC_SIZE; i++) {
            preallocatedRandoms[i] = random.nextLong();
    ...
    public static void main(String[] args) throws Exception {
        org.openjdk.jmh.runner.options.Options opt = new org.openjdk.jmh.runner.options.OptionsBuilder()
                .include(RingBufferOfLongAddAndGetBenchmark.class.getSimpleName())
                .forks(1)
                .build();
