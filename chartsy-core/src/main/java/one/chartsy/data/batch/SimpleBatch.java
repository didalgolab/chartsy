...
public class SimpleBatch<T extends Chronological> extends AbstractBatch<T> {
    ...
    public SimpleBatch(Batch<T> batch, Comparable<?> batchNumber, List<T> list) {
        this(batch.batcher(), batch.order(), batchNumber, list);
    }

    public SimpleBatch(Batcher<T> batcher, Chronological.Order order, Comparable<?> batchNumber, List<T> list) {
